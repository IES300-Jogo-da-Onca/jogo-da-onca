...
class Jogo {
    ...
    static getPossiveisMovimentos(x, y, ehCachorro, vetorTabuleiro) {
        ...
        if (posicao == '.' || (!ehCachorro && posicao == 'C') || (ehCachorro && posicao == 'O')) return null
        this.DIRECOES.forEach(el => {
            let novoX = x + el[0]
            let novoY = y + el[1]
            if (!this.posDentroDoTabuleiro(novoX, novoY)) return null
            if (this.movimentoInvalidoNoTriangulo(x, y, novoX, novoY)) return null
            try {
